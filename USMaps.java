...
public class USMaps {
	...
	private static PopulationQuery pq;
	private static final String FILENAME = "CenPop2010.txt";

	public static void main(final String[] args) {
		pq = new PopulationQuery(FILENAME);
		...
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				// Display early because the wait for something to appear
				// on-screen is unsettling
				...
				BoxLayout mainLayout = new BoxLayout(mainContentPane,
						BoxLayout.Y_AXIS);
				...
				System.out.println("interation pane added");
				appFrame.add(interactionPane, BorderLayout.SOUTH);

				// according to the swing documentation, one must call validate
				// after
				// adding components to a visible item -- doesn't seem to matter
				// on Windows,
	...
	static void runProgram(Component parent) {
		...
		InteractionPane.displayCensusData(result.getElementA(),
				result.getElementB());
	}
	...
	public static void pqPreprocess() {
		pq.preprocess(mapPane.getColumns(), mapPane.getRows(), getVersionNum());
	}
}
